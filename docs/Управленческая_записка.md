...
## 3. Методы моделирования, математические ядра и научные источники
...
### 3.1. Зачем в комплексе разработано несколько математических моделей?
В реальном космическом планировании невозможно ограничиться единственной оптимизационной моделью, так как лицо, принимающее решения (ЛПР), сталкивается с взаимоисключающими вызовами: **экономия бюджета** vs **гарантия выживания миссий при форс-мажоре** vs **нормативный директивный регламент**.

Для комплексного анализа в систему внедрены три независимых математических ядра:
1. **Regulatory Baseline (Регламентный эталон ТЗ)**:
   * **Назначение:** Жестко фиксированный директивный план по правилам технического задания (60% базового отбора через Earth-Core, покрытие остатка через Earth-Flex, стандартные сроки ввода ZBO и ISRU).
   * **Зачем нужен:** Служит обязательной «базой отсчета» (Base Case). Без неизменного эталона регламента невозможно объективно измерить добавленную стоимость (Value Added) оптимизационных алгоритмов и доказать экономию бюджета для проверяющей комиссии.
2. **NASA MILP Engine (Оптимизационное ядро минимальной стоимости LCC)**:
   * **Назначение:** Синтез математически оптимального глобального плана снабжения при предсказуемых параметрах спроса и штатной надежности каналов.
   * **Зачем нужен:** Находит идеальный компромисс между затратами на хранение, тарифами отбора, платой за бронь и штрафами Take-or-Pay. Обеспечивает **наивысшую финансовую эффективность программы (экономия 11.81% / 1.096 млрд у.е. NPV)**.
3. **Minimax Robust Engine (Отказоустойчивое антикризисное ядро)**:
   * **Назначение:** Робастное планирование по критерию минимакса Гермейера — Вальда для условий жесткой неопределенности.
   * **Зачем нужен:** В космосе высока вероятность аварий ракет-носителей, санкционных срывов пусков и технических задержек развертывания добывающего оборудования. В то время как экономически оптимальный план (MILP) при сильном стрессе жертвует коммерческими заказчиками, робастное ядро минимизирует **максимальный возможный физический дефицит в наихудшем сценарии**, обеспечивая беспрецедентную живучесть логистической сети (дефицит в стрессе снижается с 214.7 т до 45.5 т).

---

### 3.2. Детальный принцип работы и математический аппарат ядра NASA MILP

Ядро оптимизации базируется на классической методологии космической логистики **NASA Glenn Research Center (GRC)** и Массачусетского технологического института (MIT Space Logistics Framework — de Weck, Simchi-Levi, Ho):

#### 1. Вектор переменных решения (Decision Variables)
Для каждого года планирования $t \in [2035, 2040]$ и каждого канала доставки $ch \in \{A, B, C, D, E\}$ определены:
* $x_{ch, t} \ge 0$ — фактический физический объем отбора КРТ (тонн);
* $y_{ch, t} \ge 0$ — зарезервированная пропускная способность канала (тонн/год);
* $S_t \ge 0$ — физический складской остаток в баках ОТУ на конец года $t$ (тонн);
* $z^{ZBO} \in \{0, 1\}$ — бинарная переменная решения о реализации ZBO-модернизации ($z_t^{ZBO} = 1$ в год финансирования);
* $z^{ISRU} \in \{0, 1\}$ — бинарная переменная решения о строительстве завода Lunar-ISRU.

#### 2. Целевая функция (Objective Function)
Глобальная минимизация чистой приведенной стоимости затрат жизненного цикла (NPV LCC) при ставке дисконтирования $r = 0.08$:
$$\min_{\mathbf{x}, \mathbf{y}, \mathbf{z}} \sum_{t=2035}^{2040} \frac{OPEX_t(\mathbf{x}_t, \mathbf{y}_t, S_t) + CAPEX_t(\mathbf{z}_t)}{(1 + r)^{t - 2035}}$$

Где структура операционных затрат:
$$OPEX_t = \sum_{ch} \Big(Var\_Payment_{ch, t} + Res\_Tariff_{ch} \cdot y_{ch, t}\Big) + 0.72 \cdot \frac{S_{t-1} + S_t}{2} + OPEX^{fixed}_{t}$$

#### 3. Преодоление нелинейности Take-or-Pay (Точная линеаризация)
В правилах ТЗ функция переменного платежа нелинейна из-за оператора максимума:
$$Var\_Payment_{ch, t} = Unit\_Price_{ch} \cdot \max\big(x_{ch, t}, \; \alpha_{ch} \cdot y_{ch, t}\big)$$
где $\alpha_{Earth-Core} = 0.70$, $\alpha_{Earth-New} = 0.50$.  
Для перевода задачи в класс чистого MILP введена вспомогательная переменная эффективного объема $u_{ch, t}$, ограниченная системой линейных неравенств:
$$Var\_Payment_{ch, t} = Unit\_Price_{ch} \cdot u_{ch, t}$$
$$\begin{cases}
u_{ch, t} \ge x_{ch, t} \\
u_{ch, t} \ge \alpha_{ch} \cdot y_{ch, t}
\end{cases}$$
При минимизации целевой функции переменная $u_{ch, t}$ автоматически принимает точное значение $\max(x_{ch, t}, \alpha_{ch} y_{ch, t})$.

#### 4. Физический баланс и динамические параметры узла
* **Однократные потери на валовый throughput:**
  $$S_t = S_{t-1} + \sum_{ch} x_{ch, t} \cdot (1 - \text{loss\_rate}_t) - Served\_Demand_t$$
  где $\text{loss\_rate}_t = 0.045 - 0.033 \cdot \sum_{\tau \le t} z_\tau^{ZBO}$ (при вводе ZBO потери падают с 4.5% до 1.2%).
* **Динамическая емкость баков:**
  $$S_t \le 70.0 + 50.0 \cdot \sum_{\tau \le t} z_\tau^{ZBO} \quad (\text{расширение до 120.0 т})$$
* **Условие 45-дневного страхового буфера:**
  $$S_{t-1} + y_{Emergency, t} \ge \left\lceil Demand\_Total_t \times \frac{45}{365} \right\rceil$$
* **Ограничение мощности каналов:**
  $$y_{ch, t} \le Max\_Capacity_{ch}, \quad x_{ch, t} \le Max\_Capacity_{ch}$$
* **Инвестиционные потолки бюджета:**
  $$\sum_{t=2035}^{2037} CAPEX_t \le 1800.0 \text{ млн у.е.}, \quad \sum_{t=2035}^{2040} CAPEX_t \le 2800.0 \text{ млн у.е.}$$

#### 5. Алгоритмический решатель (Branch-and-Bound)
Задача решается модифицированным симплекс-методом с ветвлением и границами (Branch-and-Bound). Это гарантирует нахождение **строгого глобального минимума**, исключая попадание в локальные оптимумы, характерные для генетических алгоритмов или градиентных эвристик. Время полного расчета плана на 6–11 лет составляет менее **15 миллисекунд**.

---

### 3.3. Принцип работы Minimax Robust и Regulatory Baseline

#### Minimax Robust Optimization (Бен-Таль & Немировский)
В отличие от MILP, минимизирующего средневзвешенные затраты, робастное ядро решает минимаксную задачу:
$$\min_{\mathbf{y}, \mathbf{z}} \max_{\mathbf{\xi} \in \mathcal{U}} \left[ \text{Deficit}(\mathbf{y}, \mathbf{z}, \mathbf{\xi}) + \lambda \cdot LCC(\mathbf{y}, \mathbf{z}, \mathbf{\xi}) \right]$$
где $\mathcal{U}$ — полиэдральное множество наихудших возмущений среды:
* $\xi_{Core} \in [0.85, 1.0]$ — коэффициент срыва земных поставок;
* $\xi_{ISRU} \in [0.55, 1.0]$ — коэффициент технологической недопоставки лунного кислорода;
* $\xi_{Price} \in [1.0, 1.25]$ — коэффициент тарифного скачка.

Алгоритм заблаговременно контрактует увеличенную бронь по гибкому каналу Earth-Flex и сохраняет повышенный физический неснижаемый запас на ОТУ, создавая гарантированную буферную подушку.

---

### 3.4. Фундаментальные научные источники
1. **de Weck O., Simchi-Levi D. et al.** *"Space Logistics: A Framework for Analysis and Optimization"*, AIAA Journal of Spacecraft and Rockets.
2. **Ho K., de Weck O.** *"Dynamic Space Logistics Network Optimization using Generalized Multi-Commodity Network Flows"*, Acta Astronautica.
3. **Floudas C. A.** *"Nonlinear and Mixed-Integer Optimization: Fundamentals and Applications"*, Oxford University Press.
4. **Ben-Tal A., El Ghaoui L., Nemirovski A.** *"Robust Optimization"*, Princeton Series in Applied Mathematics.
5. **Saaty T. L.** *"The Analytic Hierarchy Process"*, McGraw-Hill (Многокритериальная оценка компромиссов стейкхолдеров).
